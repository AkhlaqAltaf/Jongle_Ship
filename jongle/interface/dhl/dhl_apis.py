...
def dhlServices():
    service = DHLService(api_key=settings.DHL_API_KEY, api_secret=settings.DHL_API_SECRET,
                        ...
                        test_mode=True)
    logging.debug('DHL Service created')
    return service
...
def calculate(sender, receiver, product, shipment_date, with_customs, unit_of_measurement):
    dhl_service = dhlServices()
    rates = dhl_service.get_rates(product, sender, receiver, shipment_date, with_customs, unit_of_measurement)
    logging.debug('Rates received: %s', rates)
    if rates.success:
        logging.debug('Rates found: %s', rates)
    else:
        logging.error('Failed to get rates: %s', rates)
        if hasattr(rates, 'error_message'):
            logging.error('Error message: %s', rates.error_message)
    return rates
